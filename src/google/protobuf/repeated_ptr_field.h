...
class RepeatedPtrFieldWithArenaBase {
 public:
  ...
  const RepeatedPtrFieldBase& field() const { return field_; }
  RepeatedPtrFieldBase& field() { return field_; }

 private:
  friend class InternalMetadataOffset;

  // A method to statically verify the offset of `field_`. We need to define
  // this in a member function out of line because RepeatedPtrFieldWithArenaBase
  // needs to be fully defined to call offsetof, but `field_` is private.
  //
  // This method does not need to be called.
  [[maybe_unused]] constexpr void VerifyLayout();
...
constexpr void RepeatedPtrFieldWithArenaBase::VerifyLayout() {
  static_assert(
      offsetof(RepeatedPtrFieldWithArenaBase, field_) == 0,
      "field_ must be at offset 0 in RepeatedPtrFieldWithArena. There are "
      "multiple places throughout the code (e.g. reflection, "
      "VerifyHasBitConsistency) which assume that you can find a "
      "RepeatedPtrFieldBase by following a pointer to a split field. By "
...
class RepeatedPtrFieldWithArena {
 ...
 private:
  ...
  // define this in a member function out of line because
  // RepeatedPtrFieldWithArena needs to be fully defined to call offsetof, but
  // `field_storage_` is private.
  //
  // This method does not need to be called.
  [[maybe_unused]] constexpr void VerifyLayout();
...
template <typename Element>
constexpr void RepeatedPtrFieldWithArena<Element>::VerifyLayout() {
  static_assert(
      offsetof(RepeatedPtrFieldWithArena, field_storage_) == 0,
      "field_storage_ must be at offset 0 in RepeatedPtrFieldWithArena. There "
      "are multiple places throughout the code (e.g. reflection, "
      "VerifyHasBitConsistency) which assume that you can find a "
      "RepeatedPtrField<Element> by following a pointer to a split field. By "
      "placing `field_storage_` at offset 0 in this struct, this assumption "
      "holds.");
}
