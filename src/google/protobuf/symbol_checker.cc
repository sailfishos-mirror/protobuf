...
std::string SymbolCheckerError::ErrorMessage() const {
  switch (type_) {
...
std::vector<SymbolCheckerError> SymbolChecker::CheckSymbolVisibilityRules() {
  ...
  for (auto& nested : nested_messages_) {
...
void SymbolChecker::VisitType(const FileDescriptor&,
                              const FileDescriptorProto& proto) {}
void SymbolChecker::VisitType(const FieldDescriptor& field,
                              const FieldDescriptorProto& proto) {}

void SymbolChecker::VisitType(const EnumValueDescriptor&,
                              const EnumValueDescriptorProto& proto) {}
void SymbolChecker::VisitType(const OneofDescriptor&,
                              const OneofDescriptorProto& proto) {}
void SymbolChecker::VisitType(const Descriptor::ExtensionRange&,
                              const DescriptorProto::ExtensionRange& proto) {}

void SymbolChecker::VisitType(const MethodDescriptor&,
                              const MethodDescriptorProto& proto) {}

void SymbolChecker::VisitType(const ServiceDescriptor& service,
                              const ServiceDescriptorProto& proto) {}

// Populate VisibilityCheckerState for messages.
void SymbolChecker::VisitType(const Descriptor& message,
                              const DescriptorProto& proto) {
  if (message.containing_type() != nullptr) {
    nested_messages_.push_back(internal::DescriptorAndProto{&message, &proto});
  }
}

// Populate VisibilityCheckerState for enums.
void SymbolChecker::VisitType(const EnumDescriptor& enm,
                              const EnumDescriptorProto& proto) {
  if (enm.containing_type() != nullptr) {
    if (SymbolChecker::IsNamespacedEnum(enm)) {
      namespaced_enums_.push_back(
          internal::EnumDescriptorAndProto{&enm, &proto});
    } else {
      nested_enums_.push_back(internal::EnumDescriptorAndProto{&enm, &proto});
    }
  }
}
