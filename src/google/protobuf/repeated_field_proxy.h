...
// `SetElementImpl` would be ambiguous for certain argument types, such a type
// which is implicitly convertible to both `std::string` and
// `absl::string_view`. With ranked overloads, the highest rank function is
// chosen in the case of ambiguity.
//
// See go/ranked-overloads
struct Rank0 {};
struct Rank1 : Rank0 {};
struct Rank2 : Rank1 {};

// Explicitly enumerate all types we accept as arguments to `set` for strings.
//
// These methods are used for both `std::string` and `absl::string_view`
// repeated fields. Were we not to explicitly list all supported assignment
// types, we would implicitly accept any type assignable to `std::string` for
// both types, as `std::string` backs both currently. At some point, we may
// change the backing storage of `absl::string_view` fields to something other
// than `std::string`.
inline void SetElementImpl(Rank2, std::string& element, const char* value) {
  element = value;
}
inline void SetElementImpl(Rank2, std::string& element,
                           std::reference_wrapper<std::string> value) {
  element = value;
}
inline void SetElementImpl(Rank1, std::string& element, std::string&& value) {
  element = std::move(value);
}
inline void SetElementImpl(Rank0, std::string& element,
...
inline void SetElement(std::string& element, T&& value) {
  SetElementImpl(Rank2{}, element, std::forward<T>(value));
}
...
template <typename ElementType, typename Enable = void>
class RepeatedFieldProxyBaseWithSet
    : public RepeatedFieldProxyBase<ElementType> {
 public:
  ...
  //
  // `index` must be less than `size()`, otherwise behavior is undefined.
  void set(size_type index, ElementType value) const {
...
template <typename ElementType>
class RepeatedFieldProxyBaseWithSet<
    ElementType, std::enable_if_t<std::is_class_v<ElementType>>>
    : public RepeatedFieldProxyBase<ElementType> {
 public:
  using Base = internal::RepeatedFieldProxyBase<ElementType>;
  using Base::Base;
  ...
  // `index` must be less than `size()`, otherwise behavior is undefined.
  void set(size_type index, ElementType&& value) const {
    Base::field()[index] = std::move(value);
  }
};

template <>
class RepeatedFieldProxyBaseWithSet<absl::string_view>
    : public RepeatedFieldProxyBase<absl::string_view> {
 public:
  using Base = internal::RepeatedFieldProxyBase<absl::string_view>;
  using Base::Base;
  using typename Base::size_type;
  ...
  void set(size_type index, T value) const {
    string_util::SetElement(Base::field()[index], std::forward<T>(value));
  }
};

template <>
class RepeatedFieldProxyBaseWithSet<std::string>
    : public RepeatedFieldProxyBase<std::string> {
 public:
  using Base = internal::RepeatedFieldProxyBase<std::string>;
  using Base::Base;
  using typename Base::size_type;

  // Sets the element at the given index to the given value.
  //
  // Prefer to `std::move` `std::string` values to avoid unnecessary copying.
  //
  // `index` must be less than `size()`, otherwise behavior is undefined.
  template <typename T>
  void set(size_type index, T value) const {
...
template <>
class RepeatedFieldProxyBaseWithSet<absl::Cord>
    : public RepeatedFieldProxyBase<absl::Cord> {
 public:
  using Base = internal::RepeatedFieldProxyBase<absl::Cord>;
  using Base::Base;
  using typename Base::size_type;

  // Sets the element at the given index to the given value.
  //
  // Prefer to `std::move` `absl::Cord` values to avoid unnecessary copying.
  //
  // `index` must be less than `size()`, otherwise behavior is undefined.
  void set(size_type index, absl::Cord&& value) const {
    Base::field()[index] = std::move(value);
  }
  // Sets the element at the given index to the given value.
  //
  ...
  //
  // `index` must be less than `size()`, otherwise behavior is undefined.
  void set(size_type index, absl::string_view value) const {
    Base::field()[index] = value;
  }
};
...
class RepeatedFieldProxy final
    : public internal::RepeatedFieldProxyBaseWithSet<ElementType> {
  static_assert(!std::is_const_v<ElementType>);

 protected:
  using Base = internal::RepeatedFieldProxyBaseWithSet<ElementType>;
  using ProxyBase = internal::RepeatedFieldProxyBase<ElementType>;

  using typename ProxyBase::const_iterator;
  using typename ProxyBase::iterator;
  using typename ProxyBase::RepeatedFieldType;
  using typename ProxyBase::size_type;
 ...
 public:
  ...
  [[nodiscard]] reference operator[](size_type index) const {
    return ProxyBase::field()[index];
  }

  using internal::RepeatedFieldProxyBaseWithSet<ElementType>::set;
  ...
  void move_assign(RepeatedFieldProxy<ElementType> other) const {
    Base::field() = std::move(other.field());
    // Repeated field move assignment is implemented as a swap, so we still need
    // to clear `other`.
    other.clear();
  }
...
// A helper function to construct a RepeatedFieldProxy. In the internal
// namespace so external code cannot call it.
template <typename T, typename... Args>
